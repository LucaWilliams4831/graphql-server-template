...
function UniqueUnionTypes(context) {
    ...
    function validateUnionTypes(node) {
        ...
        if (duplicateTypeNode) {
            const unionDiff = (0, lodash_xorby_1.default)(node.types, duplicateTypeNode.types, 'name.value');
            const diffLength = unionDiff.length;
            if (diffLength > 0) {
                context.reportError((0, utils_1.errorWithCode)('VALUE_TYPE_UNION_TYPES_MISMATCH', `${(0, utils_1.logServiceAndType)(duplicateTypeNode.serviceName, typeName)}The union \`${typeName}\` is defined in services \`${duplicateTypeNode.serviceName}\` and \`${node.serviceName}\`, however their types do not match. Union types with the same name must also consist of identical types. The type${diffLength > 1 ? 's' : ''} ${unionDiff.map(diffEntry => diffEntry.name.value).join(', ')} ${diffLength > 1 ? 'are' : 'is'} mismatched.`, [node, duplicateTypeNode]));
            }
            return false;
        }
        if (typeFromSchema) {
            context.reportError(new graphql_1.GraphQLError((0, uniqueTypeNamesWithFields_1.existedTypeNameMessage)(typeName), node.name));
            return;
        }
        if (knownTypes[typeName]) {
            context.reportError(new graphql_1.GraphQLError((0, uniqueTypeNamesWithFields_1.duplicateTypeNameMessage)(typeName), [
                knownTypes[typeName],
                node.name,
            ]));
        }
